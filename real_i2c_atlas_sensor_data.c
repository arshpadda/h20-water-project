...
static char* cloud_data_ph;
static char* cloud_data_c;
 ...
 */
static void write_data_to_file(char* buffer, char* time, int counter, char type){
	//If the counter value is equal to one then we need to write the time as it is the first value in the new row.
	if(counter == 1){
		if(type == 'c'){
			strcpy(cloud_data_c, time);
		}
		else{
			strcpy(cloud_data_ph, time);
		}
	}
	...
	if(counter == 3){
		if(type == 'c'){
			strcat(cloud_data_c, ",");
			strcat(cloud_data_c, buffer+1);
		}
		else{
			strcat(cloud_data_ph, ",");
			strcat(cloud_data_ph, buffer+1);
		}
	}
	else{
		if(type == 'c'){
 ...
 */
static void display_write_to_file(char* buffer, int counter,char type){
	if(buffer[0] == 254){
		...
		printf(" : %s", buffer+1);
		write_data_to_file(buffer,time_buffer,counter, type);
		printf("\n");
	}
}
...
static void *read_write(void *arguments){
	...
	display_write_to_file(data->buffer, data->counter,data->type);
}
		...
		switch (option){
			...
			case 4:
					...
					while(start_time < end_time){
						...
						cloud_data_ph = (char*)calloc(254,sizeof(char));
						cloud_data_c = (char*)calloc(254, sizeof(char));
						...
						s_send(publisher, cloud_data_ph);
						s_send(publisher, cloud_data_c);
						printf("\n");
